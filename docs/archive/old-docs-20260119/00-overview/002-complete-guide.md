...
### Phase 8: CloudFlare Tunnel + MetalLB (2026-01-18)

**목적**: 로컬 nginx 제거하고 CloudFlare Tunnel + MetalLB로 HTTPS 제공

**왜 필요한가?**:
- 로컬 nginx 의존성 제거 (단일 실패 지점)
- Public IP 노출 없이 HTTPS 제공
- MetalLB로 Kubernetes 네이티브 LoadBalancer 사용
- 보안 향상 (Private IP만 사용)

#### 8.1 아키텍처 변경

**이전 (Phase 7):**
```
사용자 → CloudFlare → 로컬 nginx (443) → K8s Ingress (31852 NodePort)
```

**현재 (Phase 8):**
```
사용자 → CloudFlare Tunnel → MetalLB (192.168.1.200:80) → K8s Ingress
```

**핵심 차이:**
- nginx 제거 ✅
- NodePort 제거, LoadBalancer 사용 ✅
- Private IP만 사용 (192.168.1.200) ✅

#### 8.2 MetalLB 설치

**왜 MetalLB?**:
- Bare-metal 환경에서 LoadBalancer 타입 서비스 제공
- AWS/GCP의 LoadBalancer와 동일한 경험
- IP Pool 관리 (192.168.1.200-210)
...
| 리소스 | 이름 | 상태 | 노드 | 이미지 | 비고 |
|--------|------|------|------|--------|------|
| Pod | web-55fd9f7f97-cw7j4 | Running | worker1 | **ghcr.io/wlals2/blog-web:v14** | Hugo + 게시판 UI |
| Pod | web-55fd9f7f97-cz87p | Running | worker2 | **ghcr.io/wlals2/blog-web:v14** | Hugo + 게시판 UI |
| Pod | was-5cd686f586-95gp6 | Running | worker2 | ghcr.io/wlals2/board-was:v1 | Spring Boot API |
| Pod | was-5cd686f586-hqt9j | Running | worker2 | ghcr.io/wlals2/board-was:v1 | Spring Boot API |
| Pod | mysql-65f4d695d4-w4rrp | Running | worker1 | mysql:8.0 | 데이터베이스 |
| Service | ingress-nginx-controller | LoadBalancer | - | **192.168.1.200:80** | MetalLB |

**최신 변경 (Phase 8-9):**
- ✅ **v14 이미지**: Hugo Production 빌드 (livereload 제거, baseURL 정상)
- ✅ **MetalLB**: LoadBalancer 192.168.1.200 (nginx 제거, Private IP)
- ✅ **CloudFlare Tunnel**: Public IP 노출 없이 HTTPS 제공
- ✅ **게시판 UI**: https://blog.jiminhome.shop/board
...
**완료된 Phase (2026-01-18):**
- Phase 0-7: 기본 인프라 구축
- **Phase 8**: CloudFlare Tunnel + MetalLB ✅ 신규
- **Phase 9**: 게시판 UI 구현 ✅ 신규
...
### 8.0 다음 Phase 로드맵 (2026-01-18)

**🔥 Priority 1: 핵심 기능 (운영 필수)**

| Phase | 내용 | 예상 시간 | 우선순위 |
|-------|------|----------|---------|
| **Phase 10** | Prometheus + Grafana 모니터링 | 3-4시간 | **최우선** |
| **Phase 11** | MySQL 백업/복원 자동화 | 2-3시간 | 필수 |

**⚙️ Priority 2: 성능 및 안정성**

| Phase | 내용 | 예상 시간 | 의존성 |
|-------|------|----------|--------|
| **Phase 12** | HPA (Auto Scaling) | 1-2시간 | Phase 13 후 |
| **Phase 13** | Redis Session Clustering | 2-3시간 | Phase 12 전 |

**🚀 Priority 3: 고급 기능**

| Phase | 내용 | 예상 시간 | 비고 |
|-------|------|----------|------|
| **Phase 14** | Helm Chart 구성 | 3-4시간 | 배포 자동화 |
| **Phase 15** | CI/CD 개선 (GitHub Actions) | 4-5시간 | Jenkins 대체 |
| **Phase 16** | 로깅 시스템 (EFK Stack) | 4-5시간 | 통합 로그 |
| **Phase 17** | Chaos Engineering | 2-3시간 | 장애 테스트 |

**추천 순서:**
```
1️⃣ Prometheus + Grafana (모니터링 필수!)
    ↓
2️⃣ MySQL 백업/복원 (데이터 보호)
    ↓
3️⃣ Redis Session (HPA 준비)
    ↓
4️⃣ HPA 활성화 (Auto Scaling)
    ↓
5️⃣ Helm Chart (배포 간소화)
```
