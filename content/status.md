---
title: "Infrastructure Status"
date: 2026-03-18
layout: "single"
url: "/status/"
summary: "홈랩 인프라 일일 상태 보고서 — Falco, Trivy, SLO, CI/CD 통합"
showtoc: true
tocopen: false
---
# Daily Report: 2026-03-18
> 기간: 2026-03-15 ~ 2026-03-18 (3일간)
> 생성: 2026-03-18 07:00:20

---

## 🔴 Critical Summary


- 🔴 Falco CRITICAL 3건 발생
- 🔴 비정상 Pod 존재
- 🟡 SLO 30d 89.1% (목표 99%)

---

## 🛡️ 보안 이벤트 (Falco)

| Priority | 건수 |
|----------|------|
| CRITICAL | 3건 |
| WARNING  | 497건 |
| ERROR    | 0건 |

### Rule별 상세

#### Critical
- `Drop and execute new binary in container`: 3건

#### Warning (상위 10개)
- `Shell Spawned in Application Container`: 448건
- `Read sensitive file untrusted`: 22건
- `Linux Kernel Module Injection Detected`: 15건
- `Clear Log Activities`: 12건

> 📌 노이즈 주의: `Read sensitive file untrusted` 중 gdm/PAM 관련은 로그인 정상 동작

---

## 🖥️ 클러스터 상태

- **노드**: 5/5 정상
- **비정상 Pod**:
  - argocd/argocd-dex-server-676c5dd554-jjj69 (Error)
- **재시작 많은 Pod (5회 이상)**:
  - kube-system/cilium-8zcqf 재시작:6회
  - kube-system/cilium-envoy-5mgnl 재시작:5회
  - kube-system/cilium-envoy-n2w2s 재시작:7회
  - kube-system/cilium-envoy-xc7q9 재시작:6회
  - kube-system/cilium-envoy-xlkn4 재시작:5회
  - kube-system/cilium-operator-69f67c-pfrx9 재시작:7회
  - kube-system/cilium-p2jd8 재시작:5회
  - kube-system/cilium-rvqbd 재시작:7회
  - kube-system/cilium-sz9zh 재시작:5회
  - kube-system/kube-controller-manager-k8s-cp 재시작:121회

---

## 📊 SLO

| 기간 | Availability | 목표 |
|------|-------------|------|
| 24h  | 97.4% | 99% |
| 30d  | 89.1% | 99% |

---

## 🔍 이미지 보안 스캔 (Trivy)

> 스캔 범위: CRITICAL + HIGH / 최근 nightly 결과

**스캔 요약**

| 대상 | 타입 | 취약점 수 | Secrets |
|------|------|----------|---------|
| `ghcr.io/wlals2/board-was:latest (alpine 3.23.3)` | alpine | 3 | - |
| `app/app.jar` | jar | 8 | - |

**발견된 취약점**: CRITICAL=1,HIGH=1

| Severity | Library | CVE |
|----------|---------|-----|
| 🟠 HIGH | gnutls | CVE-2026-1584 |
| 🔴 CRITICAL | zlib | CVE-2026-22184 |

---

## 🚀 CI/CD 이력 (최근 3일)

- ✅ `03-17 17:59` Deploy WAS to Kubernetes
- ✅ `03-17 04:07` Deploy WEB to Kubernetes
- ✅ `03-17 02:01` Update Homepage Metrics
- ✅ `03-16 18:00` Deploy WAS to Kubernetes
- ✅ `03-16 02:22` Update Homepage Metrics
- ✅ `03-15 17:29` Deploy WAS to Kubernetes
- ✅ `03-15 02:12` Update Homepage Metrics
- ✅ `03-14 22:00` Deploy WEB to Kubernetes

---

## 📋 TODO

### 🚨 P0 (즉시)
- 없음

### ⚠️ P1 (단기)
- [ ] MySQL HA 구성
- [ ] Control Plane HA

> 전체 목록: /home/jimin/docs/00-TODO.md

---

## 🔗 빠른 링크

| 도구 | URL |
|------|-----|
| Grafana | http://grafana.jiminhome.shop |
| Prometheus | http://prom.jiminhome.shop |
| Falcosidekick UI | http://falco-ui.jiminhome.shop |
| ArgoCD | http://argocd.jiminhome.shop |
