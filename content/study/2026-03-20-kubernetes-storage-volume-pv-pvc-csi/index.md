...
**Static Provisioning (전통 방식)**: PV가 먼저다.

관리자가 실제 스토리지(NFS, 외부 디스크 등)를 확보한 뒤 PV를 미리 여러 개 만들어 둔다. 개발자가 PVC를 만들면 조건에 맞는 PV를 찾아 바인딩한다.

**Dynamic Provisioning (Longhorn 방식)**: PVC가 먼저다.

개발자가 PVC만 던지면 StorageClass(Longhorn)가 자동으로 PV를 만들어 연결한다. 관리자가 PV를 미리 만들 필요 없다.

### PVC 요청 크기가 PV보다 크면 어떻게 되는가

Static Provisioning 환경에서 관리자가 10Gi PV만 만들어뒀는데 PVC로 20Gi를 요청하면:

```bash
kubectl get pvc
# NAME        STATUS    VOLUME   CAPACITY   ACCESS MODES
# my-pvc      Pending   (없음)   (없음)
```

**결과: Pending 무한 대기.** K8s는 에러를 내지 않고 조건에 맞는 PV가 나타날 때까지 기다린다. 이 PVC를 참조하는 Pod도 함께 Pending이 된다.

반대의 경우 — 5Gi를 요청했는데 남은 PV가 10Gi짜리뿐이라면, K8s는 더 큰 PV를 그냥 할당한다. 남은 5Gi는 다른 PVC에 쪼개어 줄 수 없고 버려진다.

> 작게 요청해서 큰 PV를 받는 것은 가능하지만, 크게 요청해서 작은 PV를 받는 것은 불가능하다.

Dynamic Provisioning(Longhorn) 환경에서는 요청한 크기 그대로 PV가 즉시 생성되므로 이 문제가 발생하지 않는다.
