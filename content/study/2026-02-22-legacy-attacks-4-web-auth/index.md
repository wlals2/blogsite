...
CSRF를 이해하려면 브라우저가 보내는 HTTP 요청의 실제 모습을 봐야 한다.

```
사용자가 attacker.com에서 <img> 태그를 렌더링할 때
브라우저가 자동으로 보내는 HTTP 요청:

GET /transfer?to=attacker&amount=1000000 HTTP/1.1
Host: bank.com
Cookie: session=abc123; JSESSIONID=xyz789    ← 브라우저가 자동 포함!
Referer: http://attacker.com/page.html       ← 출처가 attacker.com
Origin: http://attacker.com                   ← 출처가 attacker.com

→ bank.com 서버 입장에서:
  - Cookie가 유효한 세션 → "로그인된 사용자"
  - Referer/Origin을 검사하지 않으면 → "정상 요청"으로 처리
```
...
Host: bank.com
Cookie: session=abc123                        ← 자동 포함
Content-Type: application/x-www-form-urlencoded
Referer: http://attacker.com/page.html
Origin: http://attacker.com

to=attacker&amount=1000000

→ CSRF Token이 없으면 서버는 이 요청을 구분할 수 없다
```
...
속성:
  Path=/          → 쿠키가 전송될 URL 경로
  HttpOnly        → JavaScript에서 접근 불가 (document.cookie 차단)
  Secure          → HTTPS에서만 전송
  SameSite=Strict → 크로스 사이트 요청 시 쿠키 미전송
  Max-Age=1800    → 만료 시간 (초)

[이후 모든 요청 - 클라이언트]
GET /mypage HTTP/1.1
Host: target.com
Cookie: JSESSIONID=A1B2C3D4E5F6    ← 브라우저가 매 요청마다 자동 포함
```

**세션 ID 탈취 = 계정 탈취**인 이유:
서버는 Cookie 헤더의 JSESSIONID 값만으로 사용자를 식별한다.
이 값을 복사해서 공격자의 브라우저에 넣으면 같은 세션으로 인식한다.
...
## 6. Wireshark로 인증/세션 공격 확인하기

### Session Hijacking 탐지

HTTP 환경에서 세션 쿠키가 평문으로 전송되는 것을 확인할 수 있다.

```
# HTTP Cookie 헤더가 포함된 패킷 필터
http.cookie contains "JSESSIONID"
http.cookie contains "session"

# Set-Cookie 응답 (세션 발급 시점)
http.set_cookie contains "JSESSIONID"
```

Wireshark TCP Stream 분석:
```
...
GET /mypage HTTP/1.1
Host: target.com
Cookie: JSESSIONID=A1B2C3D4E5F6    ← 평문으로 노출!

→ HTTP 환경에서는 같은 네트워크의 누구나 이 값을 볼 수 있다
→ 이 값을 복사해서 다른 브라우저에서 사용하면 세션 탈취 성공
```

**HttpOnly/Secure 플래그 확인**:
```
# Set-Cookie에서 보안 플래그 확인
http.set_cookie
...
→ 플래그 없음 = XSS + 네트워크 스니핑 모두 가능
```

### CSRF 공격 탐지

```
# Referer/Origin이 다른 도메인인 POST 요청 (CSRF 의심)
http.request.method == "POST" && !(http.referer contains "target.com")

# 실제 캡처 예시
POST /transfer HTTP/1.1
Host: bank.com
Referer: http://attacker.com/page.html    ← 외부 사이트에서 온 POST
Origin: http://attacker.com
...
http.request.uri contains "%2e%2e"    # URL 인코딩된 ..
http.request.uri contains "%252e"      # 더블 인코딩된 .
...
Wireshark로 서버 응답의 보안 헤더 존재 여부를 확인할 수 있다.

```
서버 응답 확인:
HTTP/1.1 200 OK
Set-Cookie: session=abc; HttpOnly; Secure; SameSite=Strict  ← 세션 보호
X-Content-Type-Options: nosniff                              ← MIME 스니핑 방지
X-Frame-Options: DENY                                        ← Clickjacking 방지
Content-Security-Policy: default-src 'self'                  ← XSS 완화
Strict-Transport-Security: max-age=31536000                  ← HTTPS 강제

→ 이 헤더들이 없으면 해당 공격에 취약할 수 있다
```
