...
cover:
  image: "cover.jpg"
  alt: "공격자의 시선으로 보는 보안: 3편 - 웹 공격 Part 1 (Injection)"
  relative: true
---
...
## HTTP 요청 구조 (인젝션 공격의 전달 경로)

인젝션 공격은 HTTP 요청을 통해 서버에 도달한다.
공격 페이로드가 HTTP의 어디에 실리는지 이해해야 탐지와 방어가 가능하다.

```
HTTP 요청 구조:
┌────────────────────────────────────────────┐
│ Request Line                               │
│  GET /products?id=1 HTTP/1.1               │ ← URL 파라미터 (SQL Injection)
├────────────────────────────────────────────┤
│ Headers                                    │
│  Host: target.com                          │
│  User-Agent: Mozilla/5.0...               │ ← Header Injection
│  Cookie: session=abc123                    │ ← Session Hijacking
...
├────────────────────────────────────────────┤
│ Body                                       │
│  username=admin'--&password=anything       │ ← POST 파라미터 (SQL Injection)
└────────────────────────────────────────────┘
```
...
| 위치 | 예시 | 관련 공격 |
|------|------|----------|
| URL Path | `/download?file=../../../../etc/passwd` | Directory Traversal |
| URL Parameter | `?id=1 UNION SELECT...` | SQL Injection |
| POST Body | `username=admin'--` | SQL Injection |
| Cookie | `Cookie: session=<stolen>` | Session Hijacking |
| Header | `User-Agent: <script>...` | XSS, Log Poisoning |
...
## 5. Wireshark로 인젝션 공격 확인하기

인젝션 공격은 HTTP 트래픽 안에 들어있다. HTTPS가 아닌 환경에서는 Wireshark로 직접 확인 가능하다.

### HTTP 스트림 분석

```bash
# 캡처 시작 (HTTP 트래픽)
tshark -i eth0 -f "port 80" -w injection_capture.pcap
```

Wireshark에서 HTTP 패킷을 선택 → 우클릭 → **Follow > TCP Stream**으로 전체 요청/응답을 볼 수 있다.

### SQL Injection 탐지 필터

```
# HTTP 요청에서 SQL 키워드 포함된 패킷
http.request.uri contains "UNION" || http.request.uri contains "SELECT"
http.request.uri contains "OR%201%3D1"
http.request.uri contains "'--"

# POST Body에서 SQL 패턴
http.request.method == "POST" && frame contains "UNION"
http.request.method == "POST" && frame contains "OR 1=1"
```

캡처 예시:
```
HTTP 요청 (Follow TCP Stream):
...
→ URL 디코딩: id=1 UNION SELECT username,password FROM users--
...
# 응답에 반사된 스크립트 (Reflected XSS 확인)
http.response && frame contains "<script>alert"
```
...
### Command Injection 탐지 필터

```
# URL/Body에 쉘 메타문자 포함
frame contains "%3B"         # ; (세미콜론)
frame contains "%7C"         # | (파이프)
frame contains "%60"         # ` (백틱)
...
```
HTTPS (TLS 암호화) 사용 시:
→ Wireshark는 암호화된 트래픽만 보임
→ HTTP 내용 분석 불가

해결 방법 (테스트/디버깅 환경에서만):
1. 서버의 Private Key로 복호화
   Wireshark > Preferences > Protocols > TLS > RSA keys list
2. SSLKEYLOGFILE 환경 변수 사용
   export SSLKEYLOGFILE=~/tls_keys.log
   → 브라우저가 TLS 세션 키를 파일에 기록
   → Wireshark에서 이 파일로 복호화
3. 프록시 도구 사용 (Burp Suite, mitmproxy)
   → HTTPS를 중간에서 복호화하여 분석
```
