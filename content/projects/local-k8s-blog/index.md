---
title: "Local K8s Blog - GitOps 자동화 블로그 시스템 🚧"
date: 2026-01-16
summary: "로컬 Kubernetes에 Hugo 블로그 + Spring Boot WAS 배포, Jenkins CI/CD 구축 (진행 중)"
tags: ["kubernetes", "gitops", "hugo", "spring-boot", "jenkins", "project", "in-progress"]
categories: ["projects"]
series: ["Infrastructure Learning Journey"]
weight: 5
showtoc: true
tocopen: true
draft: false
---

# Local K8s Blog (🚧 진행 중)

> **상태**: 🚧 **진행 중** (2026.01.16 시작)
> **목표**: 로컬 K8s에 블로그 + WAS 배포, GitOps 자동화 (Git Push → 자동 배포)
> **키워드**: Bare-metal Kubernetes, Hugo, Spring Boot, Jenkins, Ingress

---

## 📋 Quick Summary

| 항목 | 내용 |
|------|------|
| **동기** | 이 블로그 자체를 Kubernetes Pod로 배포하고 싶음 |
| **목표** | Hugo 블로그 + Spring Boot Board를 로컬 K8s에 배포, Jenkins CI/CD 구축 |
| **기술 스택** | Kubernetes v1.31, Hugo, Spring Boot, MySQL, Jenkins, Ingress |
| **예상 소요 시간** | 8-10시간 (Phase별 단계적 진행) |

---

## 🎯 왜 이 프로젝트?

### Phase 3 (EKS)와의 차이점

| 항목 | Phase 3 (EKS) | Local K8s Blog |
|------|--------------|----------------|
| **환경** | AWS EKS (클라우드) | 베어메탈 Kubernetes (로컬) |
| **목적** | 프로덕션급 HA + DR | 블로그 자동 배포 + 학습 |
| **WEB** | nginx (정적 파일) | **Hugo 블로그 (이 블로그!)** |
| **WAS** | PetClinic (샘플) | Spring Boot Board (게시판) |
| **DB** | AWS RDS | MySQL Pod (Longhorn PVC) |
| **CI/CD** | Jenkins + ArgoCD | **Jenkins (GitOps)** |
| **배포 방식** | Canary (Argo Rollouts) | Rolling Update |

### 문제 상황 (현재)

**이 블로그 배포 방식:**
```
Hugo 소스 수정
  ↓
hugo build (로컬)
  ↓
Git Push
  ↓
Netlify/Cloudflare 자동 빌드/배포
  ↓
1-2분 후 반영 ✅
```

**하지만...**
- 블로그에서 "Kubernetes 전문가"라고 소개하는데
- 정작 내 블로그는 Kubernetes가 아닌 Netlify에서 실행 🤔
- **"자기가 쓰는 블로그도 Kubernetes로 운영해야 진짜 아닌가?"**

---

## 🏗️ 아키텍처

### 목표 아키텍처

```
┌─────────────────────────────────────────────────────────────────┐
│              Kubernetes Cluster (베어메탈)                       │
│              192.168.1.187:6443                                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Namespace: blog-system                                   │  │
│  │                                                            │  │
│  │  [Ingress Controller] nginx                               │  │
│  │        ↓                                                   │  │
│  │  [Ingress Rules]                                           │  │
│  │     /       → web-service (Hugo 블로그)                   │  │
│  │     /board  → was-service (Spring Boot Board)             │  │
│  │     /api    → was-service (REST API)                      │  │
│  │                                                            │  │
│  │  ┌───────────────┐     ┌───────────────┐                 │  │
│  │  │ WEB Pod       │     │ WAS Pod       │                 │  │
│  │  │ nginx:alpine  │     │ Spring Boot   │                 │  │
│  │  │ Hugo 정적파일  │     │ Board App     │                 │  │
│  │  └───────────────┘     └───────┬───────┘                 │  │
│  │                                 │                         │  │
│  │                        ┌────────▼────────┐                │  │
│  │                        │ MySQL Pod       │                │  │
│  │                        │ mysql:8.0       │                │  │
│  │                        │ PVC: Longhorn   │                │  │
│  │                        └─────────────────┘                │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Jenkins (Docker)                                         │  │
│  │                                                            │  │
│  │  Pipeline 1: Hugo Blog (Jenkinsfile-web)                  │  │
│  │    Git Push → Hugo Build → Docker → K8s Deploy           │  │
│  │                                                            │  │
│  │  Pipeline 2: Spring Boot WAS (Jenkinsfile-was)            │  │
│  │    Git Push → Maven Build → Docker → K8s Deploy          │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘

외부 접속:
http://192.168.1.187:30080/       (Ingress NodePort)
http://blog.jiminhome.shop/       (Cloudflare Tunnel)
```

---

## 🛠️ 기술 스택

### 기존 환경 (활용)

| 컴포넌트 | 버전/상태 | 비고 |
|---------|----------|------|
| **Kubernetes** | v1.31.13 | 베어메탈 멀티 노드 (51일 운영 중) |
| **CNI** | Cilium | 고성능 네트워킹 |
| **Storage** | Longhorn | 분산 스토리지 (3 replica) |
| **Monitoring** | Prometheus + Grafana | 기존 운영 중 |

### 신규 구축 (Local K8s Blog)

| 레이어 | 기술 | 선택 이유 |
|--------|------|----------|
| **Ingress** | nginx-ingress | Path-based Routing (`/`, `/board`) |
| **WEB** | Hugo + nginx:alpine | 이 블로그 자체! |
| **WAS** | Spring Boot 3.2 | 게시판 CRUD |
| **DB** | MySQL 8.0 | Longhorn PVC 영구 저장 |
| **CI/CD** | Jenkins | Docker 컨테이너로 간단 배포 |

---

## 📋 구현 계획 (Phase별)

### Phase 0: 환경 준비 (30분) - ⏳ 대기 중

**작업**:
1. Ingress Controller 설치 (nginx)
2. blog-system Namespace 생성
3. 프로젝트 디렉터리 구조 생성

**검증**:
- [ ] Ingress Controller Pod Running
- [ ] blog-system Namespace 생성 확인

---

### Phase 1: Hugo 블로그 Pod 배포 (1시간) - ⏳ 대기 중

**작업**:
1. Dockerfile 작성 (Multi-stage Build)
2. K8s Deployment/Service 작성
3. Docker 이미지 빌드
4. K8s 배포

**검증**:
- [ ] WEB Pod Running
- [ ] web-service ClusterIP 할당
- [ ] `/health` 엔드포인트 응답

---

### Phase 2: Spring Boot WAS 개발 및 배포 (3-4시간) - ⏳ 대기 중

**작업**:
1. Spring Boot 프로젝트 생성 (Spring Initializr)
2. 게시판 CRUD 기능 구현
3. Dockerfile 작성
4. K8s Deployment/Service/ConfigMap 작성
5. 배포

**검증**:
- [ ] WAS Pod Running
- [ ] ConfigMap 환경변수 주입 확인

---

### Phase 3: MySQL 배포 (30분) - ⏳ 대기 중

**작업**:
1. Secret, PVC, Deployment, Service 작성
2. Longhorn StorageClass 사용
3. 배포

**검증**:
- [ ] PVC Bound 상태
- [ ] MySQL Pod Running
- [ ] WAS → MySQL 연결 확인

---

### Phase 4: Ingress 설정 (30분) - ⏳ 대기 중

**작업**:
1. Ingress Manifest 작성 (Path Routing)
2. 배포 및 테스트

**접속 테스트**:
- [ ] `/` → Hugo 블로그
- [ ] `/board` → Spring Boot Board
- [ ] `/api/posts` → REST API

---

### Phase 5: Jenkins CI/CD 구축 (2-3시간) - ⏳ 대기 중

**작업**:
1. Jenkins Docker 실행
2. Jenkinsfile-web 작성 (Hugo 파이프라인)
3. Jenkinsfile-was 작성 (Spring Boot 파이프라인)
4. GitHub Webhook 연동 (선택)

**최종 목표**:
```
Git Push
  ↓
Jenkins 자동 트리거
  ↓
Docker 이미지 빌드
  ↓
Worker 노드로 전송
  ↓
K8s 배포
  ↓
1-2분 후 배포 완료 ✅
```

---

## 🎯 예상 성과

### 정량적 목표

| 지표 | 현재 (Netlify) | 목표 (K8s) |
|------|---------------|-----------|
| **배포 시간** | 1-2분 | **1-2분** (동일) |
| **자동화** | Git Push → Netlify | **Git Push → Jenkins** |
| **환경** | Netlify 서버 | **내 Kubernetes** |
| **제어** | Netlify 종속 | **완전한 제어** |
| **학습** | 없음 | **GitOps 학습 ✅** |

### 학습 목표

1. **실전 GitOps**: 내 블로그로 직접 경험
2. **Multi-stage Docker Build**: 이미지 크기 최적화
3. **Ingress Path Routing**: 하나의 IP로 여러 서비스
4. **PVC 영구 저장**: MySQL 데이터 영구 보관
5. **Jenkins Pipeline as Code**: Jenkinsfile 작성

---

## 📊 진행 상황 (Updated: 2026-01-16)

### 완료된 작업 ✅

1. ✅ 구현 계획 수립 (IMPLEMENTATION-PLAN.md)
2. ✅ 기술 스택 결정
3. ✅ 아키텍처 설계
4. ✅ "Why?" 문서화 (모든 기술 선택 이유 명시)

### 다음 작업 ⏳

- ⏳ Phase 0: 환경 준비 (Ingress Controller 설치)
- ⏳ Phase 1: Hugo 블로그 Pod 배포
- ⏳ Phase 2-5: 순차 진행

---

## 🔗 관련 문서

- **[구현 계획서 (IMPLEMENTATION-PLAN.md)](.claude/IMPLEMENTATION-PLAN.md)** - 상세 구현 가이드 (2000+ 줄)
- **[현재 상태 (context.md)](.claude/context.md)** - 프로젝트 현황
- **[Skills (blog-k8s.md)](.claude/skills/blog-k8s.md)** - 운영 명령어 모음

---

## 💭 왜 이 프로젝트가 중요한가?

### Phase 3 (EKS)와의 시너지

**Phase 3에서 배운 것:**
- ✅ EKS 클러스터 구축 (Terraform)
- ✅ GitOps (ArgoCD)
- ✅ Canary Deployment (Argo Rollouts)
- ✅ Multi-Cloud DR

**Local K8s Blog에서 배울 것:**
- ✅ 베어메탈 Kubernetes 운영 경험
- ✅ Ingress Controller 직접 설치/관리
- ✅ 로컬 이미지 관리 (ECR 없이)
- ✅ Jenkins Pipeline as Code

### "실전 경험"의 가치

**Phase 3 (EKS)**:
- PetClinic (샘플 앱) → 실제 사용 안 함
- AWS 환경 → 돈 들음 ($258/월)

**Local K8s Blog**:
- **내 블로그** → 매일 사용 ✅
- **로컬 환경** → 무료 ✅
- **장애 발생 시** → 실제로 트러블슈팅 필요 (진짜 경험)

---

## 🔮 이 프로젝트 이후

### 향후 확장: Homelab Services

Local K8s Blog 성공 후 확장 계획:
- Nextcloud (파일 저장소)
- Vaultwarden (비밀번호 관리)
- Grafana (모니터링 통합)
- Self-hosted Git (Gitea)

**목표**: 집 전체를 Kubernetes 클러스터로! 🏠

---

## 📝 업데이트 로그

- **2026-01-16**: 프로젝트 시작, 구현 계획 수립 완료
- **2026-01-XX**: Phase 0 완료 (예정)
- **2026-01-XX**: Phase 1 완료 (예정)

---

**작성일**: 2026-01-16
**프로젝트 상태**: 🚧 **진행 중** (Phase 0 대기)
**난이도**: ⭐⭐⭐ (Intermediate)
**예상 소요 시간**: 8-10시간
